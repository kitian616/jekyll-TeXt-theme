...
| **owa** | [Open Web Analytics](http://www.openwebanalytics.com/) |
| **custom** |  |
...
In order to use Google Analytics, set `piwik_url` to your Matomo URL and `site_id` to the Matomo ID for the site.

```yaml
analytics:
  provider: matomo
  matomo:
    piwik_url: "your-matomo-url-without-a-trailing-slash"
    site_id: "matomo-id-for-the-site"
```

### Open Web Analytics

In order to use Google Analytics, set `owa_url` to your Open Web Analytics URL and `site_id` to the Open Web Analytics ID for the site.

```yaml
analytics:
  provider: owa
  matomo:
    owa_url: "your-owa-url-without-a-trailing-slash"
    site_id: "owa-id-for-the-site"
```