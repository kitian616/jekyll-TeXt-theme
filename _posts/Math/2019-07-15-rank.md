...
> 이러한 matrix를 **regular matrix (non-singular, invertible matrix, 정칙행렬)** 라 한다.
<br>*det A* $\neq$ 0 이고 $A^{-1}$가 존재하여 결과 $\textbf{y}$에 대한 원인 $\textbf{x} = A^{-1}\textbf{y}$로 유일하게 정해진다.
<br> 한편, *det A* = 0 이고 $A^{-1}$가 존재하지 않아 여러 개의 vector를 하나의 vector로 mapping 시키는 square matrix를 **singular matrix (특이행렬)** 라 부른다.
