...
### 1) `cross_val_score()`: 각각의 validation fold에 대한 score를 반환합니다.
...
### 2) `cross_val_predict()`: 각각의 validation fold에 대한 예측값을 반환합니다. (unseen data에 대한 예측값이기 때문에 clean predict라고도 부릅니다)
...
### 3) `GridSearchCV`: Grid search를 하여 hyperparameter tuning할 수 있는 class입니다. (`n_jobs`를 통해 병렬처리가 가능하니 반드시 체크하도록 합시다!)
...
### 4) `RandomSearchCV`: Random search를 하여 hyperparameter tuning할 수 있는 class입니다. (scipy.stats의 여러가지 확률분포들을 사용할 수 있습니다)
...
### 5) `validation_curve()`: train_score와 validation_score를 같이 반환하는 함수입니다.
